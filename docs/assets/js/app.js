...
function createProductCard(item) {
  ...
  return `<article class="catalog-item"><div class="catalog-item-figure"><img src="${item.image}" alt="${item.title}"></div><div class="catalog-meta"><span class="pill">${item.category}</span>${titlePill}</div><div class="price-list"><div><strong>Precio 1:</strong> $ ${item.price1}</div><div><strong>Precio 2:</strong> $ ${item.price2}</div></div><div class="inline-actions"><a class="btn btn-secondary" href="${item.image}" download="catalogo-${item.code}.jpg">Descargar</a></div></article>`;
}
